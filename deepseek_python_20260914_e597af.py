...
def engineer(df):
    ...
    # fill missing store_size but KEEP tier/format/age
...
def target_encode(train_df, test_df, keys, target='total_sales',
                  smooth=20, name=''):
    """Out-of-fold target encoding for train, full-train encoding for test."""
    train_df = train_df.copy()
    test_df = test_df.copy()
    col = f'te_{name}'

    # Full-train stats for test
    agg = train_df.groupby(keys)[target].agg(['mean', 'count', 'std'])
    prior = GLOBAL_MEAN
    smooth_mean = (agg['mean'] * agg['count'] + prior * smooth) / (agg['count'] + smooth)
    test_df[col] = test_df.set_index(keys).index.map(smooth_mean)
    test_df[col] = test_df[col].fillna(prior)
...
def build_features(df):
    ...
    # weight: leave NaN, tree handles it natively
    ...
    # shelf visibility: KEEP zeros, add flag
    df['is_zero_visibility'] = (df['shelf_visibility'] == 0).astype(int)
    df['shelf_visibility'] = df['shelf_visibility'].fillna(-1)  # sentinel for trees
...
X_train_full = train[FEATURES_NUM].copy()
X_test_full  = test[FEATURES_NUM].copy()
for c in LOW_CARD_CATS:
    d = pd.get_dummies(full[c], prefix=c, dummy_na=True)
    X_train_full = pd.concat([X_train_full, d.iloc[:len(train)].reset_index(drop=True)], axis=1)
    X_test_full  = pd.concat([X_test_full,  d.iloc[len(train):].reset_index(drop=True)], axis=1)
...
def run_cv(model_fn, name):
    ...
    print(f"{name}: OOF RMSE (log space) = {rmse(y_log, oof):.5f}  "
          f"-> raw RMSE = {rmse(y, np.expm1(oof)):.2f}")
...
# Blend (simple average in log space, weights ~ by inverse OOF RMSE)
w = np.array([1/rmse(y_log, xgb_oof),
              1/rmse(y_log, lgb_oof),
              1/rmse(y_log, cb_oof)])
w = w / w.sum()
